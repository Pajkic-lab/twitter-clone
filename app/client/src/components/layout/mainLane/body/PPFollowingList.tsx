...
import { getPPFollowingUsersThunk } from 'store/features/utileSlice/thunk'
...
export const PPFollowingList: React.FC<{ userId: number }> = ({ userId }) => {
  ...
  const { PPfollowingListIsLoading, PPfollowingList, PPfollowingHasMore, PPfollowingOffset, PPfollowingLimit } =
  ...
  return (
    <Wrapper>
      <InfiniteScroll
        ...
        next={() => {
          void dispatch(getPPFollowingUsersThunk({ userId, PPfollowingOffset, PPfollowingLimit }))
        }}
        loader={<>loading...</>}
