...
export const PublicProfileConnectiosTrigger = () => {
  ...
  const { PPfollowerOffset, PPfollowerLimit, PPfollowingOffset, PPfollowingLimit } = useAppSelector(
    state => state.utile,
  )
  ...
  useEffect(() => {
    const getFollowersHelper = async () => {
      ...
      if (!PPfollowingOffset) {
        await dispatch(getPPFollowingUsersThunk({ userId, PPfollowingOffset, PPfollowingLimit }))
      }
    }
