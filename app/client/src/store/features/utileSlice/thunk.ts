...
export const getFollowersThunk = createAsyncThunk(
  'utile/getFollowers',
  async ({ followerOffset, followerLimit }: { followerOffset: number; followerLimit: number }, { rejectWithValue }) => {
    try {
...
export const getFollowingUsersThunk = createAsyncThunk(
  ...
  async (
    { followingOffset, followingLimit }: { followingOffset: number; followingLimit: number },
    { rejectWithValue },
  ) => {
    try {
      return await http.utile.getFollowingUsers({ followingOffset, followingLimit })
    } catch (error) {
      if (error instanceof Error) {
        if (isAxiosError(error)) {
          return rejectWithValue(error.response?.data)
...
export const getPPFollowersThunk = createAsyncThunk(
  ...
  async (
    ...
    try {
      return await http.utile.getPPFollowers({ userId, PPfollowerOffset, PPfollowerLimit })
    } catch (error) {
      if (error instanceof Error) {
        if (isAxiosError(error)) {
          return rejectWithValue(error.response?.data)
...
export const getPPFollowingUsersThunk = createAsyncThunk(
  ...
  async (
    ...
    try {
      return await http.utile.getPPFollowingUsers({ userId, PPfollowingOffset, PPfollowingLimit })
    } catch (error) {
      if (error instanceof Error) {
        if (isAxiosError(error)) {
          return rejectWithValue(error.response?.data)
