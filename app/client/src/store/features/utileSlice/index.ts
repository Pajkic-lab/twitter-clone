  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(
        ...
        (state, { payload }: PayloadAction<AxiosResponse<{ followersList: User[] }, any> | undefined>) => {
          if (payload && payload.data) {
            if (payload.data.followersList.length) {
              ...
              if (firstUserId !== null) {
                ...
                if (!res) {
                  state.followersList = [...state.followersList, ...payload.data.followersList]
      ...
      .addCase(
        ...
        (state, { payload }: PayloadAction<AxiosResponse<{ followingList: User[] }, any> | undefined>) => {
          if (payload && payload.data) {
            if (payload.data.followingList.length) {
              ...
              if (firstUserId !== null) {
                ...
                if (!res) {
                  state.followingList = [...state.followingList, ...payload.data.followingList]
                  state.followingOffset = state.followingOffset + state.followingLimit
                }
              }
            }
      ...
      .addCase(
        ...
        (state, { payload }: PayloadAction<AxiosResponse<{ userIdToFollow: number }, any> | undefined>) => {
          if (payload && payload.data) {
      ...
      .addCase(
        ...
        (state, { payload }: PayloadAction<AxiosResponse<{ userIdToUnFollow: number }, any> | undefined>) => {
          if (payload && payload.data) {
            state.followersList = state.followersList.map(user => {
          ...
          if (payload && payload.data) {
            if (payload.data.followersList.length) {
              ...
              if (firstUserId !== null) {
                ...
                const res = followersIds.includes(firstUserId)
                if (!res) {
                  state.PPfollowersList = [...state.PPfollowersList, ...payload.data.followersList]
                  state.PPfollowerOffset = state.PPfollowerOffset + state.PPfollowerLimit
                }
              }
            }
            if (payload.data.followersList.length < 1) {
          ...
          if (payload && payload.data) {
            if (payload.data.followingList.length) {
              ...
              if (firstUserId !== null) {
                ...
                const res = followingIds.includes(firstUserId)
                if (!res) {
                  state.PPfollowingList = [...state.PPfollowingList, ...payload.data.followingList]
                  state.PPfollowingOffset = state.PPfollowingOffset + state.PPfollowingLimit
                }
              }
            }
            if (payload.data.followingList.length < 1) {
