...
export const http = {
  auth: {
    ...
    signIn({
      email,
      password,
    }: SignInEmailRequestDto): Promise<AxiosResponse<HttpResponse<SignInEmailResponseDto>>> {
    ...
    authUser(): Promise<AxiosResponse<HttpResponse<AuthenticationResponseDto>>> {
  ...
  user: {
    ...
    getMostPopularUsers(): Promise<AxiosResponse<HttpResponse<MostPopularUsersResponseDto[]>>> {
      return httpClient.get('user/most-popular');
    },
    getSearchedUser({
      searchData,
    }: SearchUserRequestDto): Promise<AxiosResponse<HttpResponse<SearchUsersResponseDto[]>>> {
  ...
  social: {
    getSocialStats(): Promise<AxiosResponse<HttpResponse<SocialStatsResponseDto>>> {
    ...
    getFollowers({
      followerOffset,
      followerLimit,
    }: FollowerListRequestDto): Promise<AxiosResponse<HttpResponse<FollowerListResponseDto[]>>> {
      return httpClient.get(`social/followers/${followerOffset}/${followerLimit}`);
    },
    getFollowingUsers({
      followingOffset,
      followingLimit,
    }: FollowingListRequestDto): Promise<AxiosResponse<HttpResponse<FollowingListResponseDto[]>>> {
      return httpClient.get(`social/following/${followingOffset}/${followingLimit}`);
    },
